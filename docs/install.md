...
that language's version and no say in its own. BMX is 0.12 and Burxt is 1.3; one number could not
honestly carry both.
...
dependency  bmx   https://github.com/andrecorugda/bmx         burxt-0.12.0  # star needs this
```
...
release channel, and BMX is 0.12 with two implementations by one author. Copy the file instead —
...
two numbers above; the same distinction applies here, one runtime down. `tests/portability.bx` reads the
...
and target BMX 0.12 quite happily.
